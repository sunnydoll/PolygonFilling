...
//cout<<i<<endl;
		for (int j = 0; j < xyLnIndex; j++)
		{
			...
			else if(xyOp[j][3] > xyOp[j][1] && (i < xyOp[j][3] && i >= xyOp[j][1]) && scanLine[count - 1][i] != j)
			{
...
//cout<<scanLineCount[i]<<endl;
		...
		for(int j = 0; j < scanLineCount[i]; j++)
		{
			...
			{
				if(yOpPath[lIndex][k] == i && sortedInter[count - 1][i] != (xOpPath[lIndex][k] - 1))
				{
					sortedInter[count][i] = xOpPath[lIndex][k];
//cout<<sortedInter[count][i]<<" "<<count<<" ";
					count++;
					scanCount[i]++;
				}
			}
		}
//cout<<scanCount[i]<<endl;
	}
	for(int i = yMax; i >= yMin; i--)
	{
		for(int j = 0; j < scanLineCount[i]; j++)
		{
			...
			{
				int temp = 0;
				if(sortedInter[k][i] > sortedInter[k + 1][i] && sortedInter[k + 1][i] != -1 && sortedInter[k][i] != -1)
				{
//cout<<temp<<" ";
...
//cout<<i<<endl;
		for (int j = 0; j < 501; j++)
		{
			if(sortedInter[count][i] != -1 && realWorldCoor[j][i] == 0 && j > sortedInter[count][i] && j < sortedInter[count + 1][i])
			{
//cout<<"In "<<j<<" ";
				realWorldCoor[j][i] = 1;
...
//cout<<endl;
	...
	//	for (int j = 0; j < 501; j++)
	//	{			
	//		if(realWorldCoor[j][i] == 1 && (scanFst[j][i] != 1 && scanSnd[j][i] != 1) && realWorldCoor[j + 2][i] != 1)
	//		{
	//			count++;
	//		}
	//		/*else if(realWorldCoor[j + 2][i] != 1)
	//		{
	//			count++;
	//		}*/
	//		else if(count % 2 == 1)
